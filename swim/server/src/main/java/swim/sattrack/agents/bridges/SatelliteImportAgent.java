...
import swim.sattrack.agents.connectors.DataImportAgent;
import swim.structure.Value;
import swim.structure.Record;
import swim.uri.Uri;
import swim.sattrack.configUtil.ConfigEnv;
 ...
 * seconds.
 */
public class SatelliteImportAgent extends DataImportAgent {
    ...
    */
    ...
    public void didStart() {
        super.didStart();
        System.out.println("Satellite Import Agent: Agent started");
    ...
    public void processCsvData() {
        ...
        for(Integer i=0; i < this.csvData.size(); i++) {
            ...
            String noradCatId = rowItem.get("NORAD_CAT_ID").stringValue();
            // String airportType = rowItem.get("type").stringValue().replace("\"", "");

            command(Uri.parse("warp://127.0.0.1:9001"), Uri.parse("/satellite/" + noradCatId), Uri.parse("addSatellite"), rowItem);
